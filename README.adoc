...
Il suffit de lister les items qui valident l'assertion disant que leur attribut time est supérieur à 24h. Il faudra ensuite s'assurer qu'ils aient bien la note "[LATE!]".
...
Cette ligne de commande permet de démarrer un conteneur à partir d'une image. L'option it permet de garder la console de lancement ouverte sur l'écoute du port du container, et créer un bash au sein du container afin d'accéder aux fichiers du docker. L'option -e permet d'initialiser une variables d'environnement pour le docker (ici le mot de passe). L'option -p permet de choisir le port d'accès au container. Pour finir le nom de l'image que l'on souhaite dans notre container (posgres).
