...
MURRAY-VIDAL Alice et SCHNEIDER Mickael
...
Le code n'est pas structuré en couches, nous n'observons qu'un seul package qui contient toutes les sources. Cela peut poser un problème de lisibilité et donc de maintenance de la solution notamment si elle vient à évoluer avec des ajouts de fonctionnalités.
La classe TodoItem servant à la fois d'entité JPA et de DTO ferait donc partie de la couche infrastructure et service. Les 3 autres classes feraient partie d'une couche isolée (TodoItemRepository dans la couche infrastructure, TodoListController dans la couche service et TodoListApplication dans une autre catégorie avec la spécificté d'être un point d'entrée de spring).


*Où se trouve le code métier (voir la règle de gestion RG 1) ?*
Le code métier se trouve dans TodoListController.

*Cette règle est-elle facilement testable par un test unitaire ?*
Il suffit de lister des items, un  qui valide l'assertion disant que l'attribut time est supérieur à 24h et un autre non. Il faudra ensuite s'assurer que le premier a bien la note "[LATE!]" et que l'autre non.
Ce test sera faisable après avoir utilisé la fonction todoItems() qui permet d'ajouter la note aux items.
...
Cette ligne de commande permet de démarrer un conteneur. L'option -it permet de garder la console de lancement ouverte sur l'écoute du port du container, et créer un bash au sein du container afin d'accéder aux fichiers du docker. L'option -e permet d'initialiser une variable d'environnement pour le docker (ici le mot de passe). L'option -p permet de choisir le port d'accès au container. Pour finir le nom de l'image que l'on souhaite pour notre container (posgres).

* Compléter le code manquant dans la méthode `TodoListController.createTodoItem()`

*Pourquoi `todoItemRepository` est-il `null` ? Quelle est la meilleure façon de l'injecter ?*
`todoItemRepository` est null car il faut l'injecter. La meilleur façon de le faire est depuis le constructeur.
...
L'architecture hexagonale permet de décomposer une application en plusieurs composants qui communiquent entre eux grace à des ports. Cette décomposition a pour principe d'isoler la partie métier de la partie technique. Cela a pour but d'entres autres faciliter les possibles évolutions en permettant de modifier un composant sans impacter les autres et les tests.
...
Pour les adapteurs, nous utiliserons des tests d'intégration.
